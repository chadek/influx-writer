...
class Inverter:
    ...
    def PolDataInverter(self):
        try:
            inverter_data = subprocess.check_output([self.conf["inverterPoller"]["venv"], self.conf["inverterPoller"]["path"], "-p", "/dev/hidraw0", "-o", "json", "-c", "QPIGS"])
        ...
        if "validity_check" in response: 
            raise ValueError("Response unexpected: {}".format(response["validity_check"]))
        return response  
    
    def PolConfInverter(self):
        try:
            inverter_data = subprocess.check_output([self.conf["inverterPoller"]["venv"], self.conf["inverterPoller"]["path"], "-p", "/dev/hidraw0", "-o", "json",  "-c", "QPIRI"])
        except subprocess.CalledProcessError as e:
            syslog.syslog(syslog.LOG_ERR, 'inverter_poller returned with error {}'.format(e))
            raise e
        response = json.loads(inverter_data.decode('utf-8'))
    ...
    def PolFlagInverter(self):
        try:
            inverter_data = subprocess.check_output([self.conf["inverterPoller"]["venv"], self.conf["inverterPoller"]["path"], "-p", "/dev/hidraw0", "-o", "json",  "-c", "QFLAG"])
        except subprocess.CalledProcessError as e:
            syslog.syslog(syslog.LOG_ERR, 'inverter_poller returned with error {}'.format(e))
            raise e
        response = json.loads(inverter_data.decode('utf-8'))
    ...
    def PolWarningInverter(self):
        try:
            inverter_data = subprocess.check_output([self.conf["inverterPoller"]["venv"], self.conf["inverterPoller"]["path"], "-p", "/dev/hidraw0","-o", "json", "-c", "QPIWS"])
        except subprocess.CalledProcessError as e:
            syslog.syslog(syslog.LOG_ERR, 'inverter_poller returned with error {}'.format(e))
            raise e
    ...
    def ApplyInverterConf(self):
        if "battery_type" in self.conf["inverter_conf"]:
            if self.conf["inverter_conf"]["battery_type"] == "AGM":
                inverter_data = subprocess.check_output([self.conf["inverterPoller"]["venv"], self.conf["inverterPoller"]["path"], "-p", "/dev/hidraw0","-o", "json", "-c", "PBT00"])
            elif self.conf["inverter_conf"]["battery_type"] == "Flooded":
                inverter_data = subprocess.check_output([self.conf["inverterPoller"]["venv"], self.conf["inverterPoller"]["path"], "-p", "/dev/hidraw0","-o", "json", "-c", "PBT01"])
            elif self.conf["inverter_conf"]["battery_type"] == "User":
                inverter_data = subprocess.check_output([self.conf["inverterPoller"]["venv"], self.conf["inverterPoller"]["path"], "-p", "/dev/hidraw0","-o", "json", "-c", "PBT02"])
            else:
                syslog.syslog(syslog.LOG_WARNING, 'battery_type not found {}'.format(self.conf["inverter_conf"]["battery_type"]))
 
        if "charger_source_priority" in self.conf["inverter_conf"]:
            if self.conf["inverter_conf"]["charger_source_priority"] == "Utility first":
                inverter_data = subprocess.check_output([self.conf["inverterPoller"]["venv"], self.conf["inverterPoller"]["path"], "-p", "/dev/hidraw0","-o", "json", "-c", "PCP00"])
            elif self.conf["inverter_conf"]["charger_source_priority"] == "Solar first":
                inverter_data = subprocess.check_output([self.conf["inverterPoller"]["venv"], self.conf["inverterPoller"]["path"], "-p", "/dev/hidraw0","-o", "json", "-c", "PCP01"])
            elif self.conf["inverter_conf"]["charger_source_priority"] == "Solar + utility":
                inverter_data = subprocess.check_output([self.conf["inverterPoller"]["venv"], self.conf["inverterPoller"]["path"], "-p", "/dev/hidraw0","-o", "json", "-c", "PCP02"])
            elif self.conf["inverter_conf"]["charger_source_priority"] == "Only solar charging permitted":
                inverter_data = subprocess.check_output([self.conf["inverterPoller"]["venv"], self.conf["inverterPoller"]["path"], "-p", "/dev/hidraw0","-o", "json", "-c", "PCP03"])
            else:
                syslog.syslog(syslog.LOG_WARNING, 'charger_source_priority type not found {}'.format(self.conf["inverter_conf"]["device_charger_priority"]))

        if "output_source_priority" in self.conf["inverter_conf"]:
            if self.conf["inverter_conf"]["output_source_priority"] == "Utility first":
                inverter_data = subprocess.check_output([self.conf["inverterPoller"]["venv"], self.conf["inverterPoller"]["path"], "-p", "/dev/hidraw0","-o", "json", "-c", "POP00"])
            elif self.conf["inverter_conf"]["output_source_priority"] == "Solar first":
                inverter_data = subprocess.check_output([self.conf["inverterPoller"]["venv"], self.conf["inverterPoller"]["path"], "-p", "/dev/hidraw0","-o", "json", "-c", "POP01"])
            elif self.conf["inverter_conf"]["output_source_priority"] == "SBU first":
                inverter_data = subprocess.check_output([self.conf["inverterPoller"]["venv"], self.conf["inverterPoller"]["path"], "-p", "/dev/hidraw0","-o", "json", "-c", "POP02"])
            else:
                syslog.syslog(syslog.LOG_WARNING, 'output_source_priority type not found {}'.format(self.conf["inverter_conf"]["battery_type"]))
        ...
        try:
            if pe_option != "PE":
                inverter_data = subprocess.check_output([self.conf["inverterPoller"]["venv"], self.conf["inverterPoller"]["path"], "-p", "/dev/hidraw0","-o", "json", "-c", pe_option])
            if pd_option != "PD":
                inverter_data = subprocess.check_output([self.conf["inverterPoller"]["venv"], self.conf["inverterPoller"]["path"], "-p", "/dev/hidraw0","-o", "json", "-c", pd_option])
            
        except subprocess.CalledProcessError as e:
            syslog.syslog(syslog.LOG_ERR, 'inverter_poller returned with error {} runnig PE: {} or PD: {} command: '.format(e, pe_option, pd_option))
            raise e 
        ...
        payload = [
            {
                ...
                "fields": {
                    ...
                    "battery_voltage_to_steady_while_charging": data['is_battery_voltage_to_steady_while_charging']
                }
            },
            {
                "measurement": "pv",
        ...
        payload = [
            {
                ...
                "fields": {
                    ...
                    "battery_float_charge_voltage": data['battery_float_charge_voltage'],
        ...
        payload = [
            {
                ...
                "fields": {
                    ...
                    "battery_too_low_to_charge_warning": data['battery_too_low_to_charge_warning'],
                    "battery_under_shutdown_warning": data['battery_under_shutdown_warning'],
                    "battery_voltage_to_high_fault": data['battery_voltage_to_high_fault'],
                    ...
                    "inverter_voltage_too_high_fault": data['inverter_voltage_too_high_fault'],
                    "inverter_voltage_too_low_fault": data['inverter_voltage_too_low_fault'],
    ...
    def Run(self):
        ...
        while True:
            try:
                ...
                syslog.syslog(syslog.LOG_ERR, 'Failed to poll data from inverter {}'.format(e))
                failCount += 1
            ...
            if deadline < now:
                try:
                    ...
                    syslog.syslog(syslog.LOG_ERR, 'Failed to poll config from inverter {}'.format(e))
                    failCount += 1            
            ...
            if failCount == 0:
                ...
                if deadline < now:
                    ...
                    self.InfluxWrite(payload)           
                    syslog.syslog(syslog.LOG_INFO, "send config payload {}".format(payload))

                    if sorted(self.inverter_current_conf.items()) != sorted(rawConf.items()):
                        self.ApplyInverterConf()
                        payload = self.MapConfig(rawConf)
                        self.InfluxWrite(payload)

                    if sorted(self.inverter_warning.items()) != sorted(rawWarn.items()) or deadline < now:
                        self.inverter_warning = rawWarn
                        payload = self.MapWarning(rawWarn)
                        self.InfluxWrite(payload)

            elif failCount%10 > 8:
                syslog.syslog(syslog.LOG_ERR, 'Inverter polling failed in a {} time in a raw'.format(failCount))
            time.sleep(1)
